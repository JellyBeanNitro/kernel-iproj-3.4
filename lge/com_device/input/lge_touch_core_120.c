...
static void touch_work_func_a(struct work_struct *work)
{
	...
	if(ts->gf_ctrl.stage & GHOST_STAGE_1){
		if (ts->ts_data.total_num) {
			int index = ts->ts_data.total_num;
			...
			while(index--){
				if (unlikely(touch_debug_mask & DEBUG_GHOST)) 	
					TOUCH_INFO_MSG("ts->ts_data.curr_data[%d].id [%d], ts->ts_data.curr_data[%d].x_position[%d] \n", index, ts->ts_data.curr_data[index].id , index, ts->ts_data.curr_data[index].x_position);
				if(ts->ts_data.curr_button.state == 0 && /*ts->ts_data.curr_data[index].id == index &&*/ (ts->ts_data.curr_data[index].x_position == 0))
				{
